...
from src.models import DenseNet121MultiModal
...
def validate(model, val_loader, criterion, device):
    ...
    with torch.no_grad():
        for images, tabular, labels in tqdm(val_loader, desc="Validation"):
            images = images.to(device)
            tabular = tabular.to(device)
            labels = labels.to(device)
...
def plot_training_curves(train_losses, val_losses, train_aucs, val_aucs):
    ...
    plt.xlabel("Epoch")
    plt.ylabel("Loss")
    plt.title("Training and Validation Loss MM")
    plt.legend()
    ...
    plt.xlabel("Epoch")
    plt.ylabel("AUC-ROC")
    plt.title("Training and Validation AUC-ROC MM")
    plt.legend()

    plt.tight_layout()
    plt.savefig("results/plots/training_curves_mm.png")
    plt.close()
...
def main():
    ...
    model = DenseNet121MultiModal(num_classes=15, freeze_backbone=True).to(device)
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        if val_loss < best_val_loss:
            ...
            file_path = cfg.artifacts / "densenet121_mm_best.pth"
    ...
    # Save final model
    final_model_path = cfg.artifacts / "densenet121_mm_final.pth"
