#from https://github.com/yiskw713/RISE
import numpy as np
import torch
...
class RISE(nn.Module):
    def __init__(self, model, n_masks=10000, p1=0.1, input_size=(224, 224),
                 initial_mask_size=(7, 7), n_batch=128, mask_path=None):
        super().__init__()
        self.model = model
    ...
    def forward(self, x):
        ...
        for i in tqdm(range(0, self.n_masks, self.n_batch), desc="RISE Processing Batches"):
...
class CXR_RISE(RISE):
    def __init__(self, model, tabular_data, n_masks=10000, p1=0.1, input_size=(224, 224),
                 initial_mask_size=(7, 7), n_batch=128, mask_path=None):
        super().__init__(model, n_masks, p1, input_size, initial_mask_size, n_batch, mask_path)
    ...
    def forward(self, x):
        # x: input image. (1, 3, H, W)
        device = x.device
        tabular_tensor = self.tabular_data.to(device)
        ...
        for i in tqdm(range(0, self.n_masks, self.n_batch), desc="RISE Processing Batches"):
            ...
            tabular_batch = tabular_tensor.repeat(batch_size, 1)



            # out = self.model(input)
            out = self.model(input, tabular_batch)
